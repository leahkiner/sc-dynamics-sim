...
def run(show_plots):
    ...
    thetaRef = (36 * macros.D2R) + array1ThetaInit2  # [rad]

    # Determine the angle and angle rate at the end of the ramp segment/start of the coast segment
    thetaDDotMax = np.abs(thetaRef - thetaInit) / ((tCoast2 * tRamp) + (tRamp * tRamp))

    array1MaxRotAccelList2 = [0.0,
                              thetaDDotMax,
                              thetaDDotMax,
                              thetaDDotMax,
                              thetaDDotMax,
    ...
    for i in range(numArrayElements):
        array1ElementList[i].prescribedTransMotionInMsg.subscribeTo(array1ElementTransMotionMessage)
        array1ElementList[i].r_FcF_F = [- (2/3) * radiusArray * np.cos(72 * macros.D2R) * np.sin(18 * macros.D2R), 0.0, - (2/3) * radiusArray * np.cos(72 * macros.D2R) * np.cos(18 * macros.D2R)]  # [m]
        ...
        array1ElementMessageData = messaging.HingedRigidBodyMsgPayload()
        array1ElementMessageData.theta = thetaRef  # [rad]
        array1ElementMessageData.thetaDot = 0.0  # [rad/s]
    ...
    # Initialize the prescribedRot1DOF module configuration data
    thetaInit = thetaRef  # [rad]
    thetaRef = (36 * 2 * macros.D2R) + array1ThetaInit2  # [rad]
    ...
    # Initialize the prescribedRot1DOF module configuration data
    thetaInit = thetaRef  # [rad]
    thetaRef = (36 * 3 * macros.D2R) + array1ThetaInit2  # [rad]
    ...
    # Initialize the prescribedRot1DOF module configuration data
    thetaInit = thetaRef  # [rad]
    thetaRef = (36 * 4 * macros.D2R) + array1ThetaInit2  # [rad]
    ...
    # Set the simulation time
    scSim.ConfigureStopTime(macros.sec2nano(simTime1 + 4 * simTime2))
    ...
    # Initialize the prescribedRot1DOF module configuration data
    thetaInit = thetaRef  # [rad]
    thetaRef = (36 * 5 * macros.D2R) + array1ThetaInit2  # [rad]
    ...
    # Set the simulation time
    scSim.ConfigureStopTime(macros.sec2nano(simTime1 + 5 * simTime2))
    ...
    # Initialize the prescribedRot1DOF module configuration data
    thetaInit = thetaRef  # [rad]
    thetaRef = (36 * 6 * macros.D2R) + array1ThetaInit2  # [rad]
    ...
    # Set the simulation time
    scSim.ConfigureStopTime(macros.sec2nano(simTime1 + 6 * simTime2))
    ...
    # Initialize the prescribedRot1DOF module configuration data
    thetaInit = thetaRef  # [rad]
    thetaRef = (36 * 7 * macros.D2R) + array1ThetaInit2  # [rad]
    ...
    # Set the simulation time
    scSim.ConfigureStopTime(macros.sec2nano(simTime1 + 7 * simTime2))
    ...
    # Initialize the prescribedRot1DOF module configuration data
    thetaInit = thetaRef  # [rad]
    thetaRef = (36 * 8 * macros.D2R) + array1ThetaInit2  # [rad]
    ...
    # Set the simulation time
    scSim.ConfigureStopTime(macros.sec2nano(simTime1 + 8 * simTime2))
    ...
    # Initialize the prescribedRot1DOF module configuration data
    thetaInit = thetaRef  # [rad]
    thetaRef = (36 * 9 * macros.D2R) + array1ThetaInit2  # [rad]
    ...
    # Set the simulation time
    scSim.ConfigureStopTime(macros.sec2nano(simTime1 + (9 * simTime2) + 10))
    ...
    scSim.ConfigureStopTime(macros.sec2nano(simTime1 + (9 * simTime2) + 10 + simTime3))
    ...
    thetaRef = - (36 * macros.D2R) + array2ThetaInit2  # [rad]

    array2MaxRotAccelList2 = []
    # Determine the angle and angle rate at the end of the ramp segment/start of the coast segment
    thetaDDotMax = np.abs(thetaRef - thetaInit) / ((tCoast4 * tRamp) + (tRamp * tRamp))

    array2MaxRotAccelList2 = [0.0,
                              thetaDDotMax,
                              thetaDDotMax,
                              thetaDDotMax,
                              thetaDDotMax,
    ...
    for i in range(numArrayElements):
        ...
        array2ElementMessageData = messaging.HingedRigidBodyMsgPayload()
        array2ElementMessageData.theta = thetaRef  # [rad]
        array2ElementMessageData.thetaDot = 0.0  # [rad/s]
        array2ElementRotMessageList2.append(messaging.HingedRigidBodyMsg().write(array2ElementMessageData))
    ...
    # Set the simulation time
    simTime4 = tDeploy4  # [s]
    scSim.ConfigureStopTime(macros.sec2nano(simTime1 + (9 * simTime2) + 10 +  simTime3 + simTime4))
    ...
    # Initialize the prescribedRot1DOF module configuration data
    thetaInit = thetaRef  # [rad]
    thetaRef = - (36 * 2 * macros.D2R) + array2ThetaInit2  # [rad]
    ...
    # Set the simulation time
    scSim.ConfigureStopTime(macros.sec2nano(simTime1 + (9 * simTime2) + 10 + simTime3 + (2 * simTime4)))
    ...
    # Initialize the prescribedRot1DOF module configuration data
    thetaInit = thetaRef  # [rad]
    thetaRef = - (36 * 3 * macros.D2R) + array2ThetaInit2  # [rad]
    ...
    # Set the simulation time
    scSim.ConfigureStopTime(macros.sec2nano(simTime1 + (9 * simTime2) + 10 + simTime3 + (3 * simTime4)))
    ...
    # Initialize the prescribedRot1DOF module configuration data
    thetaInit = thetaRef  # [rad]
    thetaRef = - (36 * 4 * macros.D2R) + array2ThetaInit2  # [rad]
    ...
    # Set the simulation time
    scSim.ConfigureStopTime(macros.sec2nano(simTime1 + (9 * simTime2) + 10 + simTime3 + (4 * simTime4)))
    ...
    # Initialize the prescribedRot1DOF module configuration data
    thetaInit = thetaRef  # [rad]
    thetaRef = - (36 * 5 * macros.D2R) + array2ThetaInit2  # [rad]
    ...
    # Set the simulation time
    scSim.ConfigureStopTime(macros.sec2nano(simTime1 + (9 * simTime2) + 10 + simTime3 + (5 * simTime4)))
    ...
    # Initialize the prescribedRot1DOF module configuration data
    thetaInit = thetaRef  # [rad]
    thetaRef = - (36 * 6 * macros.D2R) + array2ThetaInit2  # [rad]
    ...
    # Set the simulation time
    scSim.ConfigureStopTime(macros.sec2nano(simTime1 + (9 * simTime2) + 10 + simTime3 + (6 * simTime4)))
    ...
    # Initialize the prescribedRot1DOF module configuration data
    thetaInit = thetaRef  # [rad]
    thetaRef = - (36 * 7 * macros.D2R) + array2ThetaInit2  # [rad]
    ...
    # Set the simulation time
    scSim.ConfigureStopTime(macros.sec2nano(simTime1 + (9 * simTime2) + 10 + simTime3 + (7 * simTime4)))
    ...
    # Initialize the prescribedRot1DOF module configuration data
    thetaInit = thetaRef  # [rad]
    thetaRef = - (36 * 8 * macros.D2R) + array2ThetaInit2  # [rad]
    ...
    # Set the simulation time
    scSim.ConfigureStopTime(macros.sec2nano(simTime1 + (9 * simTime2) + 10 + simTime3 + (8 * simTime4)))
    ...
    # Initialize the prescribedRot1DOF module configuration data
    thetaInit = thetaRef  # [rad]
    thetaRef = - (36 * 9 * macros.D2R) + array2ThetaInit2  # [rad]
    ...
    # Set the simulation time
    scSim.ConfigureStopTime(macros.sec2nano(simTime1 + (9 * simTime2) + 10 + simTime3 + (9 * simTime4) + 10))
    ...
    omega_BN_B_rot_rigid_data_file = open(r"/Users/leahkiner/Desktop/GNCBreck2024/DataForPlotting/omega_BN_B_rot_rigid_data.txt", "w+")
    np.savetxt(omega_BN_B_rot_rigid_data_file, omega_BN_B)
    omega_BN_B_rot_rigid_data_file.close()
